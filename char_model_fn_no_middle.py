...
class ModelConfig(object):
    def __init__(self):
        ...
        self.vocab_size = 1005 #config['vocab_size']
        ...
        self.vocab_file = "./word2vecmodel.txt.demo"
...
def network_fn(features, labels, mode, params):
    ...
    query_mp1_embedding = tf.stack([query_embedding, neigb_embedding[1]], axis=1)
    query_mp1_embedding = tf.reduce_mean(query_mp1_embedding, axis=1)
    query_mp2_embedding = tf.stack([query_embedding, neigb_embedding[3]], axis=1)
    ...
    title_mp1_embedding = tf.stack([title_embedding, neigb_embedding[5]], axis=1)
    title_mp1_embedding = tf.reduce_mean(title_mp1_embedding, axis=1)
    title_mp2_embedding = tf.stack([title_embedding, neigb_embedding[7]], axis=1)
